...
submissions/<Company>/<Role>/
  offer/                 # created on demand
    ...
    <naming.offer.analysis>  # structured review (pattern + extension)
```
...
1. Read workspace-root `.career-pipeline.yml` for `compensation.*`, preferences, background, and `naming.offer.analysis` (plugin `docs/config.md`).
...
3. If the offer analysis file already exists under `offer/`: **stop**, show the path, ask refresh/overwrite vs leave alone. Do not re-review unless the user confirms refresh.
...
6. Write `offer/<pattern>.<extension>` per career-pipeline `offer-review-format`. Present accept/decline guidance in chat.
